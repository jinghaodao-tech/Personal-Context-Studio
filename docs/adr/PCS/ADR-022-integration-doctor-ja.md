...
**実機でのend-to-end検証(このサンドボックスではなく):** push(commit `b3195f0`)、MeTheory側のpinned commit更新、ユーザー自身のマシンでの実際の`npm install`を経て、`personal-context-studio/integration-doctor`が正常に解決され、MeTheoryの`npx tsc --project tsconfig.json --noEmit`がエラー0で通った -- サンドボックス内でずっとブロックしていたエラー(`Cannot find module 'personal-context-studio/integration-doctor'`)は、実ネットワークでは消えた。MeTheory側の`npm audit`は最初、このリポジトリで`overrides`により既に直したはずの高深刻度脆弱性2件(`sharp`、`adm-zip`)がまだ出た -- npmの`overrides`はその`npm install`の**トップレベル**のパッケージにしか効かず、依存として取り込まれる側には効かないため。PCS自身の`overrides`はPCS自身のインストールを直しただけで、MeTheoryがPCSをインストールする際には何も効いていなかった。MeTheory自身の`package.json`に同じ`overrides`ブロックを追加して修正、再`npm install`でMeTheory側も脆弱性0になった。このリポジトリを今後git依存として使う消費者は誰でも同じ理由で同じ`overrides`ブロックをコピーする必要がある -- READMEか、3つ目のコネクタが増えたときのフォローアップADRに書いておく価値がある。
...
このギャップは、実コネクタがすでに2つ存在する今だからこそ重要になる: MeTheory(`read_snapshot`、`submit_template_request`)とdev-pace(`submit_import`、定期実行されるデイリーパイプライン経由。「Sequencing」参照)は、どちらもPCSを作った本人が、実際の統合を通して修正される前の「記憶頼りの契約理解」で書いたもの。決定的な適合性レポートがあれば、そのズレがデバッグセッションになる前に検出できる。
...
dry-run書き込みprobeとcompatibility matrixまで含む5-checker全体を今作っても、checker 5(Semantic Invariant)は依然としてDoctorがまだ実際に診断したことのない書き込み側の失敗モードに対して、検証済みではなく想像で設計することになる: dev-paceは実在し、実際に動いている`submit_import`コネクタだが(下記参照)、まだConnector Manifestを持たず、その契約形状(`IntegrationImportV1`)にはchecker 4のレンジ判定が使う`contractRevision`に相当するフィールドが無い -- checker 1〜4が構築・検証されたanalysis-snapshotフローとは異なる。このADRでは以下の順序を固定する:
...
- **v0.2の前に:** 実際に`submit_import`を呼ぶコネクタはすでに存在する -- dev-pace(`dev-pace_public/pcs-adapter/adapter.py`)。ローカルのRust製CLI(`dev-pace`、非公開repo)が生のウィンドウ操作ログをローカルJSONLに記録し、`tools/aggregate_activity.py`がそれをアプリ名レベルの日次集計だけに縮約する(そのrepo自身のADR-001で定めたプライバシー境界により、ウィンドウタイトル・ファイル名・URLは端末外に出ない)。`pcs-adapter/adapter.py`が各日次集計を`IntegrationImportV1`形式のペイロード(`active_minutes`、`ai_conversation_minutes`、`deep_thinking_minutes`、`window_switch_count`、`idle_minutes`、`away_minutes`、`hourly_active_minutes`)に変換し、`/v1/integration-imports`に`POST`する。`run_daily_pipeline.ps1`が集計と送信を繋ぎ、`register-daily-task.ps1`がそれをWindowsのスケジュールタスクとして登録する。この節が以前主張していた内容は誤りで、架空の2つ目のコネクタを新たに考える必要は無かった。まだ本当に足りていないのは、このADRのv0.1スコープ外の別作業として管理される: (1) dev-pace用のConnector Manifest(`connectorId: "dev_pace"`、`permissions.required: ["submit_import"]`、`capabilities: { submitImport: true, ... }`)、(2) import専用コネクタをDoctor側でサポートする変更 -- `IntegrationImportV1`には`contractRevision`/`schemaVersion`に相当するフィールドが無いため、analysis-snapshotフローを前提に作られた`ConnectorManifest.pcsContract`とchecker 4のレンジチェックを、一律必須ではなくオプション(または`capabilities.readSnapshot`条件付き)にする必要がある。加えて、レンジ比較を行わず`validateIntegrationImport`をラップするだけの、既存のsnapshot用contract checkと対になる`checkImportContract`を追加する。この両方が揃って初めて、Capability Probeのimport側dry-runと書き込みに紐づくSemantic Invariantチェックが想像上のものでなくなる。

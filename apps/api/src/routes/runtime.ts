...
export async function handleRuntimeRoute(request: IncomingMessage, response: ServerResponse, url: URL, context: RuntimeRouteContext): Promise<boolean> {
  const { db, send, body, text, activeExternalAiConsent, destinationHost, detectOllama, detectOpenAiCompatible, localAiProvider, localAiRuntime, analysisSnapshot, integrationAuthorization } = context;
  ...
  if (request.method === "GET" && url.pathname === "/v1/context/analysis-snapshot") { const profileId = url.searchParams.get("profileId"); const authorization = integrationAuthorization(db, request, "read_snapshot", profileId ?? undefined); if (!authorization.ok) { send(response, authorization.status, { error: authorization.error }); return true; } if (!profileId) { send(response, 400, { error: "profile_required" }); return true; } send(response, 200, analysisSnapshot(profileId, url.searchParams.get("from") ?? undefined, url.searchParams.get("to") ?? undefined, url.searchParams.get("timezone") ?? "UTC")); return true; }
  return false;
}

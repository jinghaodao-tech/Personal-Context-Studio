...
import { classifySemanticSensitivity } from "../semanticTaxonomy.ts";
import { detectSecretLike } from "../autoConfirm.ts";
...
export async function handleContentRoute(
  ...
  if (request.method === "GET" && parts[0] === "v1" && parts[1] === "reviews" && parts[2] === "entries" && parts[3]) { const item = db.prepare("SELECT e.id,c.document_id,c.provider,c.source_content_hash,d.content_hash,d.file_path FROM context_entries e JOIN context_entry_candidates c ON c.entry_id=e.id JOIN context_documents d ON d.id=c.document_id WHERE e.id=? AND e.status='active'").get(parts[3]) as any; if (!item) { send(response, 404, { error: "review_entry_not_found" }); return true; } const values = (db.prepare("SELECT v.id,v.field_key,v.value_json,v.sharing,v.sensitivity,v.recorded_at,f.label,f.description FROM context_values v JOIN context_template_fields f ON f.template_id=(SELECT template_id FROM context_entries WHERE id=v.entry_id) AND f.field_key=v.field_key WHERE v.entry_id=? AND v.user_confirmed=0 AND v.confirmation_mode='user_confirmed' AND v.reviewed_at IS NULL ORDER BY v.field_key").all(parts[3]) as any[]).map((value) => { const taxonomy = classifySemanticSensitivity(`${value.field_key} ${value.label ?? ""} ${value.description ?? ""}`); const decoded = decodedJson(value); const secretDetected = detectSecretLike(`${value.field_key} ${value.label ?? ""} ${value.description ?? ""}`, typeof decoded === "string" ? decoded : JSON.stringify(decoded)); return { ...value, value_json: decoded, secretDetected, taxonomyMatches: taxonomy, reviewDisposition: secretDetected ? "secret_review" : taxonomy.some((match) => match.disposition === "on_hold") ? "on_hold" : taxonomy.length > 0 ? "include" : "none" }; }); send(response, 200, { item: { ...item, stale: item.source_content_hash !== item.content_hash }, values }); return true; }

...
import { integrationReviewScript } from "./integrationReview.ts";
import { integrationReviewEditsScript } from "./integrationReviewEdits.ts";
import { integrationReviewDecisionsScript } from "./integrationReviewDecisions.ts";
...
<style>${dashboardStyles}</style></head><body><main class="shell"><header class="top"><div><div class="brand">Personal Context Studio</div><div class="muted">確定済みの個人コンテキストを安全に管理するローカル画面</div></div><div class="actions"><button id="refresh">更新</button><button class="active" id="newEntry">記録する</button></div></header><nav id="tabs" aria-label="管理画面のセクション"></nav><div id="notice" class="notice" role="status" aria-live="polite"></div><section id="metrics" class="grid" aria-label="概要"></section><section id="content" class="panel"></section></main><div id="dialog" class="dialog hidden" role="presentation"></div><script>${dashboardClientScript}</script><script>${integrationReviewScript}</script><script>${integrationReviewEditsScript}</script><script>${integrationReviewDecisionsScript}</script></body></html>`;

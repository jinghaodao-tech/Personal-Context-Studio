...
function exportPreview(profileId: string, format: string, destination = "", options: { target?: unknown; maximumCharacters?: unknown; maximumTokens?: unknown } = {}) {
  ...
  const renderFields = safe.map((row) => ({ label: row.label, fieldKey: row.field_key, value: decodedValue(row), purpose: profile.purpose_id ?? "general", recordedAt: row.recorded_at, provenance: row.source_id ?? row.source, confirmationState: row.user_confirmed ? "confirmed" : "pending", lastReviewedAt: row.reviewed_at, lastReconfirmedAt: row.last_reconfirmed_at, reconfirmAfter: row.reconfirm_after, lifecycle: row.lifecycle_state, limitations: row.sensitivity === "sensitive" ? "sensitive value; use only for the selected purpose" : "none", applicability: (db.prepare("SELECT applicability_condition AS condition,valid_from AS validFrom,valid_to AS validTo FROM context_value_applicability WHERE value_id=? ORDER BY valid_from").all(row.id) as Array<{ condition: string | null; validFrom: string | null; validTo: string | null }>) }));
...
export const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (await handleOperationsRoute(request, response, url, parts, { db, databasePath, backupDirectory, watcherStatePath, encryptionKey: dataEncryptionKey, send, now, newId, fileHash, audit, provenance })) return;
     ...
     if (await handleContentRoute(request, response, url, parts, { db, send, body, text, now, newId, audit, provenance, notesRoot, readMarkdownSnapshot, excerpt, ftsTerms, upsertDocument, templateDetail, integrationPermissions, integrationAuthorized, hashIntegrationToken, randomToken: () => randomBytes(32).toString("base64url"), decodedJson })) return;

...
function validateAnalysisValue(value: unknown): ContextAnalysisValueV2 {
  ...
  const keys = new Set((value.allowedValues ?? []).map((item) => item.key)); for (const name of ["positiveValueKeys", "orderedValueKeys"]) if (value[name] !== undefined && (!Array.isArray(value[name]) || new Set(value[name]).size !== value[name].length || value[name].some((item) => typeof item !== "string" || !keys.has(item)))) throw new Error("context_analysis_value_semantics_invalid");

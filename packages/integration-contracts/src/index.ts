...
function validateAnalysisValue(value: unknown): ContextAnalysisValueV2 {
  ...
  const type = String(value.valueType); const validValue = type === "boolean" ? typeof value.value === "boolean" : type === "single_choice" ? typeof value.value === "string" : typeof value.value === "number" && Number.isFinite(value.value) && (type !== "integer" || Number.isInteger(value.value)); if (!validValue) throw new Error("context_analysis_value_invalid");
  ...
  if (value.allowedValues !== undefined && (!Array.isArray(value.allowedValues) || new Set(value.allowedValues.map((item) => item.key)).size !== value.allowedValues.length || value.allowedValues.some((item) => !isRecord(item) || typeof item.key !== "string" || typeof item.label !== "string"))) throw new Error("context_analysis_value_choices_invalid");
  const keys = new Set((Array.isArray(value.allowedValues) ? value.allowedValues : []).map((item) => isRecord(item) ? String(item.key) : "")); for (const name of ["positiveValueKeys", "orderedValueKeys"]) if (value[name] !== undefined && (!Array.isArray(value[name]) || new Set(value[name]).size !== value[name].length || value[name].some((item) => typeof item !== "string" || !keys.has(item)))) throw new Error("context_analysis_value_semantics_invalid");
...
export function validateContextAnalysisSnapshot(value: unknown): ContextAnalysisSnapshot {
  ...
  if (value.schemaVersion === CONTEXT_ANALYSIS_SNAPSHOT_V2_VERSION && value.contractRevision === PCS_ANALYSIS_CONTRACT_REVISION && typeof value.snapshotId === "string" && typeof value.profileId === "string" && isRecord(value.period) && validTimestamp(value.period.startAt) && validTimestamp(value.period.endAt) && typeof value.period.timezone === "string" && Array.isArray(value.records) && isRecord(value.excluded)) {
    ...
    for (const record of value.records) if (!isRecord(record) || typeof record.id !== "string" || !validTimestamp(record.recordedAt) || !Array.isArray(record.values) || record.values.some((item) => { validateAnalysisValue(item); return false; })) throw new Error("context_analysis_snapshot_invalid");

...
export function validateIntegrationTemplateRequest(value: unknown): IntegrationTemplateRequestV1 {
  ...
  for (const key of ["minimumObservations", "minimumPerGroup"] as const) if (value[key] !== undefined && (typeof value[key] !== "number" || !Number.isInteger(value[key]) || value[key] < 1 || value[key] > 10000)) throw new Error("integration_template_request_invalid");
  ...
  for (const field of value.requestedFields) { const item = field as Record<string, unknown>; if (item.semanticRole !== undefined && (typeof item.semanticRole !== "string" || !keyPattern.test(item.semanticRole))) throw new Error("integration_template_request_invalid"); if (item.analysisUsage !== undefined && !["condition", "outcome", "both"].includes(String(item.analysisUsage))) throw new Error("integration_template_request_invalid"); if (item.collectionTiming !== undefined && !["task_start", "before_activity", "during_activity", "after_activity", "daily", "follow_up"].includes(String(item.collectionTiming))) throw new Error("integration_template_request_invalid"); if (item.questionText !== undefined && (typeof item.questionText !== "string" || item.questionText.length > 500)) throw new Error("integration_template_request_invalid"); if (item.sharingDefault !== undefined && !["always", "purpose_only", "private", "never"].includes(String(item.sharingDefault))) throw new Error("integration_template_request_invalid"); if (item.sensitivity !== undefined && !["normal", "sensitive", "highly_sensitive"].includes(String(item.sensitivity))) throw new Error("integration_template_request_invalid"); for (const key of ["minimum", "maximum"] as const) if (item[key] !== undefined && (typeof item[key] !== "number" || !Number.isFinite(item[key] as number))) throw new Error("integration_template_request_invalid"); if (item.minimum !== undefined && item.maximum !== undefined && Number(item.minimum) > Number(item.maximum)) throw new Error("integration_template_request_invalid"); for (const key of ["positiveValueKeys", "orderedValueKeys"] as const) if (item[key] !== undefined && (!Array.isArray(item[key]) || item[key].some((entry) => typeof entry !== "string"))) throw new Error("integration_template_request_invalid"); if (item.numericMapping !== undefined && (!isRecord(item.numericMapping) || Object.values(item.numericMapping).some((entry) => typeof entry !== "number" || !Number.isFinite(entry)))) throw new Error("integration_template_request_invalid"); }

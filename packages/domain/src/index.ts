...
export type AssertionKind = "observation" | "measurement" | "preference" | "decision" | "plan" | "external_claim" | "inference";
export const ASSERTION_KINDS: readonly AssertionKind[] = ["observation", "measurement", "preference", "decision", "plan", "external_claim", "inference"];
const conceptKeyPattern = /^[a-z][a-z0-9_]*(\.[a-z][a-z0-9_]*)*$/;
export type ContextTemplateField = { fieldKey: string; label: string; description?: string; valueType: ContextValueType; required: boolean; displayOrder: number; options?: Array<{ key: string; label: string }>; minimum?: number; maximum?: number; unit?: string; analysisRole?: string; analysisRoleConfirmed?: boolean; analysisUsage?: "condition" | "outcome" | "both" | "excluded"; analysisMergeAllowed?: boolean; positiveValueKeys?: string[]; orderedValueKeys?: string[]; numericMapping?: Record<string, number>; reconfirmationMode?: "none" | "default" | "explicit"; reconfirmationIntervalDays?: number | null; sharingDefault: Sharing; sensitivity: Sensitivity; autoConfirmOnIngestion?: boolean; conceptKey?: string; defaultKind?: AssertionKind; reason: string };
/**
 * ADR-017 precedence for an assertion's `kind`: an explicit kind on the write
 * itself (from a template-designed UI, an external tool's ingestion contract,
 * or a human's live selection) wins; otherwise fall back to the field's
 * declared `defaultKind`; otherwise the assertion has no kind (unstructured).
 * `kind` is deliberately never inferred from the value itself -- see ADR-017's
 * "Rejected alternative" for why per-value classification was rejected.
 */
export function resolveKind(explicit: unknown, fieldDefaultKind: string | null | undefined): AssertionKind | null {
  if (explicit !== undefined && explicit !== null && explicit !== "") {
    if (typeof explicit !== "string" || !ASSERTION_KINDS.includes(explicit as AssertionKind)) throw new Error("context_value_kind_invalid");
    return explicit as AssertionKind;
  }
  if (fieldDefaultKind && ASSERTION_KINDS.includes(fieldDefaultKind as AssertionKind)) return fieldDefaultKind as AssertionKind;
  return null;
}

...
test("formal SQLite migrations are idempotent and record every version", () => {
  ...
  try {
    ...
    assert.deepEqual(versions.map((row) => row.version), ["001_core_schema", "002_value_governance", "003_generic_integrations", "004_integration_clients", "005_review_sharing_backup"]);

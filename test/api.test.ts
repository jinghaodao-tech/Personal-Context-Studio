...
test("local documents feed reviewed analysis snapshots and generic integration template requests", async () => {
  ...
  assert.equal(request.response.status, 201); assert.equal(request.body.status, "pending_user_review"); const createdTemplate = await api(`/v1/integration-template-requests/${request.body.id}/create-template`, "POST"); assert.equal(createdTemplate.response.status, 201); assert.equal(createdTemplate.body.template.status, "draft"); const duplicateTemplate = await api(`/v1/integration-template-requests/${request.body.id}/create-template`, "POST"); assert.equal(duplicateTemplate.response.status, 200); const edited = await api(`/v1/integration-template-requests/${request.body.id}/approve_with_edits`, "POST", { edits: [{ fieldKey: "focus", questionText: "修正した質問" }] }); assert.equal(edited.body.status, "approved"); const requestDetail = await api(`/v1/integration-template-requests/${request.body.id}`, "GET"); assert.equal(requestDetail.body.request.reviewHistory.length, 1); const activated = await api(`/v1/integration-template-requests/${request.body.id}/activate`, "POST", { }); assert.equal(activated.body.status, "activated"); const activatedDetail = await api(`/v1/integration-template-requests/${request.body.id}`, "GET"); assert.equal(activatedDetail.body.request.reviewHistory.at(-1).action, "activate");
    ...
    const reuseEdited = await api(`/v1/integration-template-requests/${reuseRequest.body.id}/approve_with_edits`, "POST", { edits: [{ fieldKey: "energy", questionText: "再利用項目の確認質問" }], confirmedAnalysisFields: [{ fieldKey: "energy", analysisRole: "outcome", analysisUsage: "outcome", analysisMergeAllowed: true }] });
    assert.equal(reuseEdited.body.status, "approved");
    assert.equal((await api(`/v1/context-templates/${templateId}`, "GET")).body.item.fields.find((field: any) => field.field_key === "energy").question_text, "");
    const reuseActivation = await api(`/v1/integration-template-requests/${reuseRequest.body.id}/activate`, "POST");
    assert.equal(reuseActivation.body.status, "activated", JSON.stringify(reuseActivation.body));
    assert.equal((await api(`/v1/context-templates/${reuseEdited.body.templateId}`, "GET")).body.item.fields.find((field: any) => field.field_key === "energy").question_text, "再利用項目の確認質問");

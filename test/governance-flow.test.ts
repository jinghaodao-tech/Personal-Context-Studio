...
test("review, purpose-limited sharing, export history, conflicts, reconfirmation and backups form a safe flow", async () => {
  ...
  try {
    ...
    const third = await api("/v1/context-entries/candidates", "POST", { templateId: template.body.item.id, sourceDocumentId: document.body.id, provider: "local", values: { energy: 3 } });
    const fourth = await api("/v1/context-entries/candidates", "POST", { templateId: template.body.item.id, sourceDocumentId: document.body.id, provider: "local", values: { energy: 1 } });
    await api(`/v1/context-entries/${third.body.id}/values/energy/review`, "POST", { decision: "accepted", reason: "Manual resolution input" });
    await api(`/v1/context-entries/${fourth.body.id}/values/energy/review`, "POST", { decision: "accepted", reason: "Manual resolution alternative" });
    const manualConflict = (await api("/v1/context-conflicts")).body.items.find((item: any) => item.status === "unresolved");
    assert.ok(manualConflict);
    const thirdValueId = (await api(`/v1/context-entries/${third.body.id}`)).body.values[0].id;
    const manualResolution = await api(`/v1/context-conflicts/${manualConflict.id}/resolve`, "POST", { status: "resolved_manually", baseValueId: thirdValueId, newValue: 5, sharing: "purpose_only", sensitivity: "normal", validFrom: "2026-08-01T00:00:00.000Z", validTo: "2026-09-01T00:00:00.000Z", reason: "Human selected the corrected value" });
    assert.equal(manualResolution.response.status, 200);
    assert.equal(manualResolution.body.resolution.newValue, 5);
    assert.ok(manualResolution.body.resolution.newRevisionId);
    const manualRevisions = await api(`/v1/context-entries/${third.body.id}/values/energy/revisions`);
    assert.ok(manualRevisions.body.items.some((item: any) => item.change_type === "correction"));
    const fifth = await api("/v1/context-entries/candidates", "POST", { templateId: template.body.item.id, sourceDocumentId: document.body.id, provider: "local", values: { energy: 4 } });
    const sixth = await api("/v1/context-entries/candidates", "POST", { templateId: template.body.item.id, sourceDocumentId: document.body.id, provider: "local", values: { energy: 2 } });
    await api(`/v1/context-entries/${fifth.body.id}/values/energy/review`, "POST", { decision: "accepted", reason: "Keep both first" });
    await api(`/v1/context-entries/${sixth.body.id}/values/energy/review`, "POST", { decision: "accepted", reason: "Keep both second" });
